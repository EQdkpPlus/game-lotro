...
class charImporter extends page_generic {
	...
	public function ajax_massupdate(){
		$chardata	= $this->game->obj['ldata']->character($this->in->get('charname', ''), $this->config->get('servername'), true);
	...
	public function perform_step0(){
		...
		if($this->config->get('uc_lockserver') == 1){
			...
			$hmtlout .= new hhidden('servername', array('value' => stripslashes($this->config->get('servername'))));
		}else{
			$hmtlout .= new htext('servername', array('value' => (($this->config->get('servername')) ? stripslashes($this->config->get('servername')) : ''), 'size' => '25'));
		}
		$hmtlout .= '</dd>
			</dl>';
